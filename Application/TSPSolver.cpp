...
void TSPSolver::run(ConfigData& config) {
    ...
    if (!config.randomData) graph.fillGraphFromFile(config.matrixFile);
    ...
    for (int iteration = 0; iteration < config.iterations; ++iteration) {
        ...
        if (config.randomData) graph.generateRandomCostMatrix(config.numCities);
    ...
    Config appConfig;
    Config::saveResultsToCSV(config.algorithm, config.numCities, config.iterations, avgTime, cost);
